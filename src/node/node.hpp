...
public:
    ...
    Leaf(const String& name, SharedPtr<Node> parent = nullptr, const String value = {});
...
class SchemaNode : virtual public Node, public inheritable_enable_shared_from_this<SchemaNode> {
  public:
    virtual ~SchemaNode();
    SchemaNode(const String& name, SharedPtr<Node> parent = nullptr);
    void addAttr(const String attr_name, const String attr_val);
    String findAttr(const String attr_name);
    virtual void accept(Visitor& visitor) override;

  private:
    static const Set<String> ATTR_NAME;
    static const Set<String> TYPE_NAME;
    Map<String, String> m_attr_by_name; // Attribute definition by its name
};
